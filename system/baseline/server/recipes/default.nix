  ...
  sops = {
    ...
    templates = {
      tandoor-env.content = ''
        				...
        				SOCIALACCOUNT_PROVIDERS={"openid_connect":{"SCOPE":["openid","profile","email"],"OAUTH_PKCE_ENABLED":true,"APPS":[{"provider_id":"authelia","name":"Authelia","client_id":"${
              config.sops.placeholder."tandoor/client_id"
            }","secret":"${
              config.sops.placeholder."tandoor/client_secret"
            }","settings":{"server_url":"https://auth.gappyland.org/.well-known/openid-configuration","token_auth_method":"client_secret_basic"}}]}}
        			'';
